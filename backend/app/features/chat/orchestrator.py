...
async def _run_haiku_lab_extraction(
    ...
    base_prompt = (
        "You are extracting lab results from a clinical lab report "
        "(often Labcorp format). Extract EVERY individual lab result "
        "in the document.\n\n"
        "LABCORP LAYOUT — the report is organized into test sections. "
        "Each section has a header (e.g. 'CBC With Differential/"
        "Platelet', 'Comprehensive Metabolic Panel', 'Lipid Panel', "
        "'Thyroid'), followed by a column header line that reads "
        "approximately:\n"
        "  'Tests   Result   Flag   Previous Result and Date   "
        "Units   Reference Interval'\n"
        "Then one line PER MARKER. A typical result line looks like:\n"
        "  '<Marker Name>   01   <value>   [flag]   "
        "[previous value & date]   <unit>   <low>-<high>'\n"
        "Notes on parsing these lines:\n"
        "- The '01' (sometimes another two-digit code) is an internal "
        "Labcorp lab/method code — IGNORE it, it is NOT the value.\n"
        "- The numeric value immediately AFTER the lab code is the "
        "current result. If a flag (H, L, High, Low, A, etc.) follows "
        "the value, map it to status (H/High -> 'high', L/Low -> "
        "'low', otherwise 'normal').\n"
        "- A 'Previous Result and Date' column may appear between the "
        "value and the unit — do NOT mistake the previous value for "
        "the current value; always take the FIRST value after the "
        "marker name / lab code.\n"
        "- The Reference Interval is usually 'low-high' separated by "
        "a hyphen or en-dash (e.g. '13.0-17.0'). It can also be "
        "one-sided ('>59', '<150', '<=5.6') or a textual range "
        "('Not Estab.', 'Negative'). Parse numeric bounds when "
        "present; use null for a bound that isn't given or isn't "
        "numeric.\n"
        "- If no explicit flag is printed but the value falls outside "
        "the reference interval, infer status from the bounds. If the "
        "value is within bounds (or bounds are unknown) use 'normal'.\n"
        "- Ignore page headers/footers, patient demographics, "
        "specimen/accession numbers, ordering physician, and any "
        "narrative comments — only emit actual measured markers.\n"
        "- test_date is the specimen collection / report date "
        "(format YYYY-MM-DD). Use the same date for every result "
        "unless a result clearly has its own date.\n\n"
        "For each result return: marker_name, value (numeric), unit, "
        "reference_low (numeric or null), reference_high (numeric or "
        "null), status (one of 'low' | 'normal' | 'high'), and "
        "test_date.\n\n"
        "CRITICAL OUTPUT RULES: Respond with ONLY the raw JSON "
        "object. No markdown code fences, no ``` , no 'json' label, "
        "no explanation, no preamble, no trailing text. The very "
        "first character of your response must be '{' and the very "
        "last must be '}'. Use this exact shape:\n"
